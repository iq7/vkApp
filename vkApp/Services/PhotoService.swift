...
//  vkApp
//
//  Created by Андрей Тихонов on 04.05.2018.
...
class PhotoService {
    ...
    private let syncQueue = DispatchQueue(label: "ru.tihon.vkApp.photoService", qos: .userInteractive)
...
//        let index = url.index(of: "?") ?? url.endIndex
//        let newURL = url[..<index]
//        let newURLString = String(newURL)
        let newURLString = url
        var image: UIImage?
        if let photo = images[newURLString] {
            image = photo
        } else if let photo = getImageFromCache(url: newURLString) {
            image = photo
        } else {
            loadPhoto(atIndexpath: indexPath, byUrl: newURLString)
        }
        return image
    }
}
